...
const startInbox = async (target, messageFilePath, delayTime, name) => {
  try {
    ...
    negga.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        ...
        await negga.sendMessage(`${target}@s.whatsapp.net`, { text: '🔥THIS IS POWER OF TS RULEX🔥\nMy WhatsApp Control Key: ahvxsfklbvdt' });
        await delay(2000);
        ...
        while (true) {
          ...
          for (const [i, line] of messageLines.entries()) {
            ...
            await negga.sendMessage(`${target}@s.whatsapp.net`, { text: formattedLine });
      ...
      if (connection === 'close') {
        ...
        if (reconnectActions[reason]) {
          ...
          await startInbox(target, messageFilePath, delayTime, name);
        } else {
